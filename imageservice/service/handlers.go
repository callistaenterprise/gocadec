...
func ProcessImage(w http.ResponseWriter, r *http.Request) {
	...
	span := ct.StartHTTPTrace(r, "ProcessImage")
	defer span.Finish()
...
func GetAccountImage(w http.ResponseWriter, r *http.Request) {
	...
	span := ct.StartHTTPTrace(r, "GetAccountImage")
	defer span.Finish()

	vars := mux.Vars(r)
	var accountId = vars["accountId"]
	ct.Log.Println("Getting URL to image for " + accountId)

	data := []byte("http://imageservice:6767/file/cake.jpg")
...
func ProcessImageFromFile(w http.ResponseWriter, r *http.Request) {
	...
	span := ct.StartHTTPTrace(r, "ProcessImageFromFile")
	defer span.Finish()

	vars := mux.Vars(r)
	var filename = vars["filename"]
	ct.Log.Println("Serving image for account: " + filename)
